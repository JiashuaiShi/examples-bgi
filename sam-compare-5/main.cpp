...
// 截取输出文件名
string getResultFileName(const string &s1, const string &s2) {
...
vector<string> readFile(const string &filePath) {
    ...
    string line;
    vector<string> res;
    bool headFlag = true; // 表示sam文件头部结束
...
void compareHash(vector<string> data, int threshold) {
...
//#pragma omp parallel for  num_threads(120)  private(i) shared(data, isPostFix, hashMap,threshold) default(none)
    for (i = 0; i < data.size(); i++) {
        ...
        if (get<0>(it->second) == field[2]
            ...
            // 如不同，则填入
            tuple<string, uint64, string> value = make_tuple(field[2], stoi(field[3]), data[i]);
            pair<std::string, tuple<std::string, int, std::string>> e = make_pair(qName, value);
            hashMap.insert(e);
        }
    }
...
// 要在这里使用openmp
void buildHashMap(vector<string> &data) {
...
#pragma omp parallel for  num_threads(120)  private(i) shared(data, isPostFix, hashMap) default(none)
    for (i = 0; i < data.size(); i++) {
        ...
        tuple<string, int, string> value = make_tuple(field[2], stoi(field[3]), data[i]);
        pair<std::string, tuple<std::string, int, std::string>> e = make_pair(qName, value);
        hashMap.insert(e);
    }
...
int main(int argc, char *argv[]) {
    ...
    // 读取文件1
    auto s1 = readFile(samFileName1);
    auto s2 = readFile(samFileName2);
